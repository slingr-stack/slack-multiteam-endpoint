...
// TODO: OLD Slackbot endpoint: The following functions will be removed in the future
endpoint.old = {};
endpoint.old.sendMessage    = function(options){ return oldSlackFunction('chat.postMessage',options) };
endpoint.old.channel        = function(options){ return oldSlackFunction('channels.info',   options) };
endpoint.old.channels       = function(options){ return oldSlackFunction('channels.list',   options) };
endpoint.old.group          = function(options){ return oldSlackFunction('groups.info',     options) };
endpoint.old.groups         = function(options){ return oldSlackFunction('groups.list',     options) };
endpoint.old.ims            = function(options){ return oldSlackFunction('im.list',         options) };
endpoint.old.user           = function(options){ return oldSlackFunction('users.info',      options) };
endpoint.old.users          = function(options){ return oldSlackFunction('users.list',      options) };
endpoint.old.ping           = function(options){ return oldSlackFunction('auth.test',       options) };
endpoint.old.convertEvent   = function(options) {
    if(!options){
        return options;
    }
    var response = null;
    try {
        var r = endpoint.__convertEvent(options);
        if(!!r) {
            response = r;
        }
    } catch (e){
